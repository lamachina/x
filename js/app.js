...
function satsToFormattedDollarNumber(sats, _bitcoinPrice) {
    return (satToBtc(sats) * _bitcoinPrice)
}
async function processOrder(order, latestOrders, inscriptionDataCache) {
    ...
    /*     let ownershipResponse = await trac.emit('get', {
            func: 'transferable',
            args: [inscriptionData.address, 'x'],
            call_id: ''
        });
        console.log("OWNERSHIP"); */

    const contentType = inscriptionData["content type"];
    ...
    const ord = {
        ...
        inscriptionId,
        priceBTC: satToBtc(validatedPrice),
        priceDollars: satsToFormattedDollarString(validatedPrice, await bitcoinPrice),
        priceDollarsNumber: satsToFormattedDollarNumber(validatedPrice, await bitcoinPrice)
    };

    return ord;
}

async function* getLatestOrders(limit, nostrLimit = 100, filters = {}) {
    await nostrRelay.connect();
    const latestOrders = [];
    ...
    for (const order of orders) {
        try {
            const ord = await processOrder(order, latestOrders, inscriptionDataCache);
            if (ord) {
...
async function inscriptionPage() {
    ...
    const processSellerPsbt = async (_sellerSignedPsbt) => {
        ...
        if (sellerSignedPsbtBase64) {
            ...
            document.getElementById('btnBuyInscriptionNow').textContent = `Buy for ${sellPriceText}`
            /*    priceDollars: satsToFormattedDollarString(validatedPrice, await bitcoinPrice),
               priceDollarsNumber: satsToFormattedDollarNumber(validatedPrice, await bitcoinPrice) */
    ...
    try {
        ...
        for await (const order of ordersGenerator) {
            ...
            if (content.includes("x") && content.includes("transfer")) {
                const contente = JSON.parse(content);
                const priceInDollarsNumber = order.priceDollarsNumber;
                const pricePerUnitRaw = priceInDollarsNumber / contente.amt;
                const pricePerUnit = parseFloat(pricePerUnitRaw.toFixed(4));
        ...
        for (const orderObj of ordersArray) {
            ...
            <div class="card card-tertiary w-100 fmxw-300">
                <div class="card-header text-center">
                    <span>Inscription #${orderObj.order.number}</span>
                </div>
                <div class="card-body" style="padding: 6px 7px 7px 7px">
                    <div>${sanitizeHTML(orderObj.contente.amt)}</div>
                    <div class="d-flex row xPrice">
                    <div>$${orderObj.pricePerUnit} </div>
                    <div> / x</div>
                    </div>
                    <button class="btn btn-primary mt-2 icon-button" style="max-height: revert">
                        ${sanitizeHTML(orderObj.order.title)}
                        <img src="/img/xlogo.png" alt="Icon" class="icon-img">
                    </button>
                </div>
            </div>`;
